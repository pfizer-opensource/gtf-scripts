...
def read_genes_from_yaml(yaml_path: Union[str, bytes, os.PathLike]) -> list[Gene]:
    ...
    if isinstance(yaml_args, dict):
        yaml_args = yaml_args['genes']
 
    for gene_dict in yaml_args:
        transcripts = []
        for transcript_dict in gene_dict.pop('transcripts'):
            # remove 'strand' if supplied -- this is a complete hack to prevent users
            # from providing a strand as part of the exon entries in the yaml file
            for exon in transcript_dict['exons']:
                exon.pop('strand', None)
            transcripts.append(
                Transcript(
                    transcript_name = transcript_dict.pop('name'),
                    cds_start = transcript_dict.pop('cds_start'),
                    cds_end = transcript_dict.pop('cds_end'),
                    exons = [Exon(strand = gene_dict['strand'], **exon) for exon in transcript_dict.pop('exons')],
                    **transcript_dict))
        gene = Gene(
